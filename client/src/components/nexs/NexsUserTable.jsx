  ...
  const rows = useMemo(() => {
    return (users || [])
      .map((u, i) => ({
        ...
        name: pick(u, 'name', 'displayName', 'fullName', 'employeeName'),
        designation: pick(u, 'designation', 'title', 'jobTitle'),
        ...
        location: pick(u, 'location', 'site', 'facility'),
        roles: Array.isArray(u.roleGroups)
          ? u.roleGroups.join(', ')
          : Array.isArray(u.roles)
            ? u.roles.map((r) => (typeof r === 'string' ? r : r?.name)).filter(Boolean).join(', ')
            : pick(u, 'roleNames', 'roleGroup'),
        hrOnly: Boolean(u?._hrOnly),
        raw: u,
      }))
      .filter((r) => Boolean(r.name || r.email || r.phone || r.department || r.empCode || r.roles))
  }, [users])
        ...
        <tbody className={`divide-y ${nexsCx.border}`}>
          {rows.map((r) => (
            <tr key={r.key} className="hover:bg-[var(--bg3)]">
              <td className={`px-4 py-2.5 ${nexsCx.text}`}>
                <div className="flex items-baseline gap-2">
                  <span>{r.name || '—'}</span>
                  {r.hrOnly && (
                    <span
                      className="text-[10px] px-1.5 py-0.5 rounded-full"
                      style={{ color: 'var(--accent)', background: 'color-mix(in srgb, var(--accent) 14%, var(--bg3))' }}
                      title="From HR data — not yet a Nexs user. Will be created on save."
                    >
                      new
                    </span>
                  )}
                ...
                {r.designation && (
                  <div className={`text-[11px] ${nexsCx.text3}`}>{r.designation}</div>
                )}
              </td>
              ...
              <td className={`px-4 py-2.5 ${nexsCx.text2}`}>
