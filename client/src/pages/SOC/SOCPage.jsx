...
function socRangeMeta(range) {
  ...
  const rv = range?.value || SOC_DEFAULT_RANGE_VALUE
    ...
    <div style={{ display:'flex', flexDirection:'column', gap:0 }}>
      ...
      {tab==='overview' && (
        <div style={{ display:'flex', flexDirection:'column', gap:12 }}>
          <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit, minmax(118px, 1fr))', gap:10 }}>
            <KPI label="Total Events"     value={stats?.total?.toLocaleString()}   sub={`last ${range?.label||range?.value||SOC_DEFAULT_RANGE_VALUE}`}           color="blue"   delta={null} onClick={() => goToSocSearch({})} />
          ...
          <div style={{ display:'grid', gridTemplateColumns:'2fr 1fr', gap:12 }}>
            <Card title="SESSION VOLUME TREND" badge={(range?.label||range?.value||SOC_DEFAULT_RANGE_VALUE).toUpperCase()} height={200}>
      ...
      {tab==='traffic' && (
        <div style={{ display:'flex', flexDirection:'column', gap:12 }}>
          <div style={{ display:'grid', gridTemplateColumns:'repeat(6,1fr)', gap:10 }}>
            <KPI label="Total Sessions" value={stats?.total?.toLocaleString()} sub={`last ${range?.label||range?.value||SOC_DEFAULT_RANGE_VALUE}`} color="blue" onClick={() => goToSocSearch({ logtype: 'traffic' })} />
          ...
          <Card title="TRAFFIC TIMELINE" badge={(range?.label||range?.value||SOC_DEFAULT_RANGE_VALUE).toUpperCase()} height={220}>
            <Line data={timelineData} options={{ ...co, onClick: (_, els) => { if (els.length) goToSocSearch({ logtype: 'traffic' }) }, plugins:{ legend:{ display:true, labels:{ color:tc.text2, font:{ size:10 }, boxWidth:10 } } }, scales:{ ...co.scales, x:{ ...co.scales.x, ticks:{ ...co.scales.x.ticks, maxTicksLimit:tickLimit } } } }} />
          </Card>
      ...
      {tab==='vpn' && (
        <div style={{ display:'flex', flexDirection:'column', gap:12 }}>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:12 }}>
            <Card title="SESSION TIMELINE" badge={(range?.label || range?.value || SOC_DEFAULT_RANGE_VALUE).toUpperCase()} height={220}>
              <Line data={timelineData} options={{ ...co, onClick: (_, els) => { if (els.length) goToSocSearch({ logtype: 'traffic' }) }, plugins:{ legend:{ display:true, labels:{ color:tc.text2, font:{ size:10 }, boxWidth:10 } } }, scales:{ ...co.scales, x:{ ...co.scales.x, ticks:{ ...co.scales.x.ticks, maxTicksLimit:tickLimit } } } }} />
            </Card>
