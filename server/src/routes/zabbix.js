...
router.get('/rop-disconnect-export', async (req, res) => {
  try {
    const nowMs = Date.now()
    const range = String(req.query.range || '7d').toLowerCase()
    const customFromSec = parseInt(String(req.query.from || ''), 10)
    ...
    const data = await fetchRopGroupDisconnectEvents({ groupKey, fromMs, toMs, businessHours })
    const wb = await buildRopDisconnectEventsReport({
      ...data,
      tzOffsetMinutes,
    })

    const safeGroup = groupKey.replace(/[^a-z0-9_-]+/gi, '_').slice(0, 40)
    const filename = `ROP_Disconnect_Events_${safeGroup}_${new Date().toISOString().slice(0, 10)}.xlsx`
