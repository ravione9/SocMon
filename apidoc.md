...
1000 requests / minute per account (JWT user id; agent key or IP when unauthenticated) on /api/* — unlocks within 1 minute after the limit is hit. Store monitor overview and AI chat can take 30s–6min — use a long client timeout. CSV exports stream; do not expect JSON.
